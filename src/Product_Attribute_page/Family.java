  ...
  //        options.addArguments("--disable-extensions");
				 ...
				 for(int k=3; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==4)
					 	...
					 	try
					 	{
					 		...
					 		Usermenu=driver.findElement(By.xpath(".//*[@id='jqgh_family-table_MaterialFamily1']")).getText();
					 		...
					 		if("Material Family".equals(Usermenu))
					 		{
					 			sheet.getRow(218).createCell(9).setCellValue("PASS");
					 		}
					 		else
					 		{
					 			sheet.getRow(218).createCell(9).setCellValue("FAIL");
					 		}
					...
					// 	Assert.assertEquals("Material Family", Usermenu);
		         }
	}
				 ...
				 for(int k=3; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==4)
					 	...
					 	try
					 	{
				 			...
				 			je.executeScript("arguments[0].scrollIntoView(true);", driver.findElement(By.xpath(".//*[@id='add_family-table' and @title='Add new row']/div/span")));
					 		...
					 		{
					 			sheet.getRow(223).createCell(9).setCellValue("PASS");
					 		}
					 		else
					 		{
					 			sheet.getRow(223).createCell(9).setCellValue("FAIL");
					 		}
				 ...
				 for(int k=3; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==4)
					 	...
					 	try
					 	{
					 		...
					 		if(AddRecord_alertmessage.trim().contentEquals("Material Family: field is required"))
					 		{
					 			sheet.getRow(229).createCell(9).setCellValue("PASS");
					 		...
					 		{
					 			sheet.getRow(229).createCell(9).setCellValue("FAIL");
					...
					// 	Assert.assertEquals("Material Family: field is required", AddRecord_alertmessage.trim());
		         }
	}
	@Test(priority=6)
				 ...
				 for(int k=3; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==4)
					 	...
					 	try
					 	{
					 		...
					 		{
					 			sheet.getRow(235).createCell(9).setCellValue("PASS");
					 		}
					 		else
					 		{
					 			sheet.getRow(235).createCell(9).setCellValue("FAIL");
					 		}
				 ...
				 for(int k=3; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==4)
					 	...
					 	try
					 	{
					 		...
					 		if("Material Attribute".equals(User1))
					 		{
					 			sheet.getRow(241).createCell(9).setCellValue("PASS");
					 		}
					 		else
					 		{
					 			sheet.getRow(241).createCell(9).setCellValue("FAIL");
					 		}
				 ...
				 for(int k=0; k<=sheet.getLastRowNum();k++)
		         {
				 	...
				 	try
					 	...
					 	//	Company_Addrecord_button();
			 				...
			 				driver.findElement(By.xpath(".//*[@id='MaterialFamily1']")).sendKeys(Systemname);
				 ...
				 for(int k=14; k<=sheet.getLastRowNum();k++)
		         {
				 	...
				 	try
					 	...
					 	//	Company_Addrecord_button();
			 				...
			 				List<WebElement> record=driver.findElements(By.xpath(".//td[@aria-describedby='family-table_MaterialFamily1']"));
			 			...
			 			List<WebElement> record1=driver.findElements(By.xpath(".//span[@class='ui-icon ui-icon-trash']"));
			 				...
			 				if(count==1)
			 				{
			 					sheet.getRow(247).createCell(9).setCellValue("PASS");
			 				}
			 				else
			 				{
			 					sheet.getRow(247).createCell(9).setCellValue("FAIL");
			 				}
	...
	public void Enter_record_Alertmessage() throws InterruptedException
	{
			  ...
			  int counts=0;
				 for(int k=0; k<=sheet.getLastRowNum();k++)
		         {
				 	...
				 	try
					 	...
					 	//	Company_Addrecord_button();
			 				...
			 				if("Material family already exists!!".equals(AlreadyExit))
			 				{
			 					sheet.getRow(324).createCell(9).setCellValue("pass");
			 				}
			 				else
			 				{
			 					sheet.getRow(324).createCell(9).setCellValue("fail");
			 				}
				...
				// 	Assert.assertEquals("Material family already exists!", AlreadyExit);
		         }
	}
