...
public class Attribute_page_last {
				 ...
				 for(int k=3; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==4)
					 	...
					 	try
					 	{
					 		...
					 		if(AddRecord_alertmessage.trim().contentEquals("Material Brand: field is required"))
					 		{
					 			sheet.getRow(12).createCell(9).setCellValue("PASS");
					...
					// 	Assert.assertEquals("Material Brand: field is required", AddRecord_alertmessage.trim());
		         }
	}
	@Test(priority=6)
				 ...
				 for(int k=0; k<=sheet.getLastRowNum();k++)
		         {
				 	...
				 	try
					 	...
					 	//	Company_Addrecord_button();
			 				...
			 				driver.findElement(By.xpath(".//*[@id='MaterialBrand1']")).sendKeys(Systemname);
