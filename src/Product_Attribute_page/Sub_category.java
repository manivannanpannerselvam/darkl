...
public class Sub_category {
  ...
  //        options.addArguments("--disable-extensions");
				 ...
				 for(int k=3; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==4)
					 	...
					 	try
					 	{
				 			...
				 			je.executeScript("arguments[0].scrollIntoView(true);", driver.findElement(By.xpath(".//*[@id='add_subCategory-table' and @title='Add new row']/div/span")));
					 		...
					 		{
					 			sheet.getRow(189).createCell(9).setCellValue("PASS");
					 		}
					 		else
					 		{
					 			sheet.getRow(189).createCell(9).setCellValue("FAIL");
					 		}
				 ...
				 for(int k=3; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==4)
					 	...
					 	try
					 	{
					 		...
					 		if(AddRecord_alertmessage.trim().contentEquals("Material Sub Category: field is required"))
					 		{
					 			sheet.getRow(189).createCell(9).setCellValue("PASS");
					...
					// 	Assert.assertEquals("Material Sub Category: field is required", AddRecord_alertmessage.trim());
		         }
	}
	@Test(priority=6)
				 ...
				 for(int k=3; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==4)
					 	...
					 	try
					 	{
					 		...
					 		{
					 			sheet.getRow(195).createCell(9).setCellValue("PASS");
					 		}
					 		else
					 		{
					 			sheet.getRow(195).createCell(9).setCellValue("FAIL");
					 		}
				 ...
				 for(int k=3; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==4)
					 	...
					 	try
					 	{
					 		...
					 		if("Material Attribute".equals(User1))
					 		{
					 			sheet.getRow(201).createCell(9).setCellValue("PASS");
					 		}
					 		else
					 		{
					 			sheet.getRow(201).createCell(9).setCellValue("FAIL");
					 		}
				 	...
				 	try
					 	{
				 			...
				 			List<WebElement> record=driver.findElements(By.xpath(".//td[@aria-describedby='category-table_MaterialCategory1']"));
					 	...
					 	//	Company_Addrecord_button();
			 				...
			 				driver.findElement(By.xpath(".//*[@id='MaterialSubCategory1']")).sendKeys(Systemname);
	...
	public void Enter_Record() throws InterruptedException
	{
			  ...
			  int counts=0;
				 for(int k=45; k<=sheet.getLastRowNum();k++)
		         {
					 if(k==46)
				 		{
				 			break;
				 	...
				 	try
					 	...
					 	//	Company_Addrecord_button();
			 				...
			 				List<WebElement> record=driver.findElements(By.xpath(".//td[@aria-describedby='subCategory-table_MaterialSubCategory1']"));
			 				...
			 				WebElement subbrand=driver.findElement(By.xpath(".//td[@aria-describedby='subCategory-table_MaterialSubCategory1']"));
			 			...
			 			List<WebElement> record1=subbrand.findElements(By.xpath(".//span[@class='ui-icon ui-icon-trash']"));
			 				...
			 				if(count==1)
			 				{
			 					sheet.getRow(208).createCell(9).setCellValue("PASS");
			 				}
			 				else
			 				{
			 					sheet.getRow(208).createCell(9).setCellValue("FAIL");
			 				}
	...
	public void Enter_record_Alertmessage() throws InterruptedException
	{
				 ...
				 for(int k=0; k<=sheet.getLastRowNum();k++)
		         {
				 	...
				 	try
					 	...
					 	//	Company_Addrecord_button();
			 				...
			 				if("Material sub category already exists!".equals(AlreadyExits))
			 				{
			 					sheet.getRow(213).createCell(9).setCellValue("FAIL");
			 				}
			 				else
			 				{
			 					sheet.getRow(213).createCell(9).setCellValue("FAIL");
			...
			//	 	Assert.assertEquals("Material sub category already exists!", AlreadyExits);
		         }
	}
